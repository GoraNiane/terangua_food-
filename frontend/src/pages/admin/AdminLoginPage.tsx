        ...
        <div className="bg-white rounded-3xl border border-[#EAEAEA] p-7 sm:p-9 shadow-xl shadow-black/[0.02] space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full h-11 mt-2 flex items-center justify-center gap-2 rounded-xl bg-[#0A0A0A] hover:bg-[#222222] text-white text-xs sm:text-sm font-bold tracking-wide transition-all shadow-xs active:scale-98 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
            >
          ...
          <div className="pt-4 border-t border-[#F0F0F0] space-y-2.5">
            <div className="text-[11px] font-bold text-[#888888] text-center uppercase tracking-wider">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                onClick={async () => {
                  ...
                  setPassword('Restaurant@2026');
                  setIsLoading(true);
                  setErrorMessage(null);
                  const res = await login('kitchen@restaurant.com', 'Restaurant@2026', true);
                  if (res.success) {
                    navigate('/kitchen', { replace: true });
                  } else {
                    setErrorMessage(res.error || 'Erreur');
                    setIsLoading(false);
                  }
                }}
                className="px-2 py-2 rounded-xl border border-amber-200 bg-amber-50/80 hover:bg-amber-100/90 text-amber-950 font-bold text-[11px] transition-all flex flex-col items-center justify-center gap-1 cursor-pointer active:scale-95"
              >
                <span>👨‍🍳</span>
                <span>Cuisine</span>
              </button>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setIsLoading(true);
                  setErrorMessage(null);
                  const res = await login('admin@restaurant.com', 'Restaurant@2026', true);
                  if (res.success) {
                    navigate('/admin', { replace: true });
                  } else {
                    setErrorMessage(res.error || 'Erreur');
                ...
                className="px-2 py-2 rounded-xl border border-blue-200 bg-blue-50/80 hover:bg-blue-100/90 text-blue-950 font-bold text-[11px] transition-all flex flex-col items-center justify-center gap-1 cursor-pointer active:scale-95"
              >
                <span>👑</span>
                <span>Admin</span>
              </button>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setIsLoading(true);
                  setErrorMessage(null);
                  const res = await login('staff@restaurant.com', 'Restaurant@2026', true);
                  if (res.success) {
                    navigate('/admin', { replace: true });
                  } else {
                    setErrorMessage(res.error || 'Erreur');
                ...
                className="px-2 py-2 rounded-xl border border-emerald-200 bg-emerald-50/80 hover:bg-emerald-100/90 text-emerald-950 font-bold text-[11px] transition-all flex flex-col items-center justify-center gap-1 cursor-pointer active:scale-95"
              >
                <span>🛎️</span>
                <span>Salle</span>
              </button>
